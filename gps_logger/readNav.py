...
def processBlocks(lines, header, obstimes, svset, headlines, sats):
    ...
    """
    # blocks = Panel4D(blocks,            #新版的pandas库已经不支持Panal4D了
    #                  labels=obstypes,
    ...
    blocks = xr.DataArray(
        ...
        coords={
            "obstypes": obstypes,
            "svset": np.arange(max(svset) + 1),
            "obstimes": obstimes,
            "variables": ['data', 'lli', 'ssi']
        },
        dims=["obstypes", "svset", "obstimes", "variables"]
    )
    # 按照您的需求筛选维度
    blocks = blocks.sel(svset=list(svset))
    return blocks
...
def getSatXYZ(nav, sv, times,t_oc):
    """
...
if __name__ == '__main__':
    nav = readRinexNav('brdc1410.21n')
    getSatXYZ(nav,22,[21,5,21,0,5,5])